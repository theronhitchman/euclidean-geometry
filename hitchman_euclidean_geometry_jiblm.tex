...
Though the students do not really know even the most basic geometry, I do not wish to insult them by telling them so. Nor do I wish to get bogged down in building planar geometry from a modern conception of a good axiomatic system like Hilbert's \cite{Hilbert}. I do want to give them a sense of history, and to get to at least a few really beautiful theorems. To meet all of these goals at once, I use Euclid's \emph{The Elements}
\cite{Euclid} as a text book. I have the students read the first four books, which gets through material on congruence of triangles and polygons, constructions (especially of regular figures), properties of circles, and area. We must stop short of discussing similarity of figures.
...
My introduction to inquiry-based learning, and specifically the Moore Method, was in a workshop run by G. Edgar Parker. That workshop was originally designed by Stan Yoshinobu, who I met later. I am heavily indebted to Timothy McNicholl for his paper \emph{The Extreme Moore Method} \cite{mcnicholl}, which I read right after that workshop. Later, I read the book \emph{The Moore Method} \cite{cmmp}, and I attended a workshop run by Carol Schumacher and Michael Starbird. Through the Academy of Inquiry-Based Learning I got mentoring from T. Kyle Petersen and Gary Richter. I also learned some things about how to construct a set of tasks by reading the text
...
Here is how I run the first day:
\begin{description}
...
This first task has been designed with several purposes in mind. There is a discussion about the different pieces of this task and their purposes following its statement below.
...
Along the way I have to take a couple of questions about mathematics, but mostly I am trying (1) to put faces to names on the roster, and (2) to make myself more approachable.
...
After the student gets up, but before they start speaking, I interrupt and explain the basic ground rules to the class.
\begin{itemize}
\item We use only last names.
\item We are polite to a fault.
\item When in our seats, we ask questions rather than give arguments.
\item The presenter's job is to convince the class, but everyone else is trying to stay unconvinced as long as is reasonable.
\end{itemize}

We take ten to fifteen minutes to give the presentation and discuss it. 
...
At this point, we are usually just over time, so I let the class go with wishes for good luck.
\end{description}
...
As part of the basic structure of the course, I run a class journal.\footnote{I have written about this elsewhere. \cite{MAANOTES}.}
...
I provide the students with a \LaTeX\ template and guidelines for preparing their papers. 
I also give the students a basic example in the paper by Rouse Ball. (This paper contains a famous fallacy and is an elaborate set-up for Problem 27.) They submit papers to me (as the managing editor) through a dedicated email address. Each paper goes through a review and revision cycle as many times as required to get it into a reasonable shape. At first, I play the role of referee and write up little referee reports for each paper. I play referee for each student's first two papers. Later in the term, I ask the students who have two or more published papers to help out and play referee. Again, I provide the students with guidelines for being a referee. If things are going well, I often turn over the refereeing completely by week ten or eleven. 
...
\begin{figure}[h]
	\centering
    \includegraphics[width=.6\textwidth]{Euclid1_1}
...
NOTE: This section and the next addressed to the author's students. If you are not the author's student, the details in this section may not apply to you directly. Check with your
instructor about how these details will work.
...
I have typed out a paper (with an old chestnut of an argument) that models the generally accepted style and included it as an appendix.
You can use this paper as a guide for writing your first paper.
Also, there is a template file you can use to properly format your work in \LaTeX\ .\footnote{The course website has links to both the example and a template. For this paper version of the notes, I have included the example at the end.}
...
Now we take up our mathematical work. 
...
There are three natural arguments students make for this conjecture.
...
The second uses Euclid's Postulate V, and might appear after the students prove Conjecture 
\ref{conj:rhombus-is-parallelogram}.

The third is a proof by contradiction that quickly splits up into many cases.
This argument was more common for my students for a few semesters, but they needed help getting things organized.
This proof requires students to wrangle with the ideas behind ``the same side'' and ``opposite sides'' of a line.
...
If you are uncomfortable waiting so long, you might hint that they are trying to 
prove that some lines are not parallel, and ask if any of their current tools help them
show such a thing. A student who really looks might realize that Postulate V is helpful.
...
{
\color{blue}
This item and the next are straightforward items, but they are the first times that students will be required to use results about parallel lines and right angles. A discussion disambiguating Euclid's Propositions 27 through 29 of the parallel postulate would be helpful. And Conjecture \ref{conj:rectangle-opp-sides} sometimes brings an opportunity to 
talk about Eulicd's Postulate V. If not here, then that should happen somewhere in this section. Take the opportunity and talk through the unfamiliar language of Postulate V and 
its role as a constructive statement. Then, if the students have not finished Conjecture \ref{conj:rhombus-diagonals}, you might find a way to drop a hint.
...
\begin{figure}[h]
\centering
\includegraphics[width=.4\textwidth]{counterexample.png}
\caption{The sneaky counterexample.}
\end{figure}
...
It is also possible to address this when Thales' theorem relating right angles and semicircles is proved in Chapter \ref{section:triangles}.
}
\end{annotation}
...
Notice the importance that pairs of parallel lines play in these statements.\footnote{These results don't \emph{look} to be about rectangles.}
...
If you have not, yet, read Professor Ball's argument in the appendix and figure out what went wrong.
...
What might this person's mysterious objection to the argument for 
Euclid's Proposition I.7 be? Do you agree?
If so, can you fix the argument?
\end{problem}
...
What might be this person's mysterious objection to the argument for 
Euclid's Proposition I.4 be? Do you agree?
If so, can you fix the argument?
\end{problem}
...
{
\color{blue}
The trouble here is the use of superposition, and the lack of axiomatic support for that. I always lead a short clarifying discussion here about how this early proposition violates the spirit of the axiomatic method, and how modern writers often include I.4 as a postulate rather than as a theorem. (This applies to Hilbert's system, for example.)
}
\end{annotation}

So far, I have strongly hinted that the arguments for Propositions I.1, I.4 and I.7 have sometimes been judged imperfect in some way.
...
{
\color{blue}
This task allows one to construct square roots, if you have appropriately chosen a
unit segment. It doesn't hurt to point that out to students, especially if their 
modern algebra course might discuss what lengths are constructible.
}
\end{annotation}
...
{
\color{blue}
This is a version of the straightedge and compass construction of inversion in a circle.
}
\end{annotation}
...
{
\color{blue}
This is related to the extraction of a square root. If the figure is constructed so that $BD$ is a unit segment, and $DC$ is some given segment, the segment $AD$ will be the square root of $DC$. Connect this idea to Problem \ref{prob:quadrature-of-rectangle}.
}
\end{annotation}
